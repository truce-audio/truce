...
| `screenshot` | Render each plugin's GUI to `target/screenshots/`. Builds the cdylib, dlopens it, and calls the `__truce_screenshot` symbol exported by `truce::plugin!`. |
