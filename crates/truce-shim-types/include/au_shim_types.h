...
// AU shim ABI version. `AuCallbacks::abi_version` (its first field, at
// a fixed offset that must never move) reports which version the
// plugin was built against, so a v3 appex generated by a *newer*
