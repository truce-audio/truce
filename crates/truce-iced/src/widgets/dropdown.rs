...
//!
//! Identical to `SelectorWidget` — iced's `pick_list` already renders as a
//! proper dropdown popup. This module exists so the `Dropdown` widget kind
//! has a distinct type in the API, matching the other backends.
...
impl<'a, M: Clone + Debug + 'static> From<DropdownWidget<'a, M>> for Element<'a, Message<M>> {
    fn from(d: DropdownWidget<'a, M>) -> Self {
        d.into_element()
    }
}
