...
/// Host-facing AU display name. Reads `truce.toml`'s `au3_name`
/// when building for v3 (so the v3 framework dylib reports the same
/// label the appex's Info.plist declares) and `au_name` otherwise.
/// Falls back to `PluginInfo::name`. Both values are baked into
/// `PluginInfo` by `truce::plugin_info!` — no env-var hop.
fn resolved_plugin_name(info: &truce_core::info::PluginInfo) -> &'static str {
    let override_value = if cfg!(truce_au_v3_only) {
        info.au3_name.or(info.au_name)
    } else {
        info.au_name
    };
    truce_core::info::resolve_name_override(override_value, info.name)
}
