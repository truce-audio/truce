...
| License | MIT / Apache-2.0 | AGPLv3 / commercial | ISC | WDL/zlib-like | ISC |
| First release | 2026 | 2004 | 2022 | 2018 (rewrite of iPlug) | 2014 |
| Maturity | Early | Production-proven | Active (pre-1.0) | Active | Active |

## Plugin Format Support

| | truce | JUCE | nih-plug | iPlug2 | DPF |
|---|---|---|---|---|---|
| CLAP | Yes | Community fork (official in JUCE 9) | Yes | Yes | Yes |
| VST3 | Yes | Yes | Yes | Yes | Yes |
| VST2 | Yes | Deprecated | No | Yes | Yes |
| AU v2 | Yes | Yes | No | Yes | Yes |
| AU v3 | Yes | Yes | No | Yes | No |
| AAX | Yes | Yes | No | Yes | No |
| LV2 | Yes | Yes (since JUCE 7) | No | No | Yes (primary) |
| LADSPA / DSSI | No | No | No | No | Yes |
| Standalone | Yes (cpal) | Yes | Yes (JACK) | Yes | Yes (JACK) |
| Web Audio | No | No | No | Yes (Emscripten / WAM) | No |
| Formats total | 7 | 6 | 2 | 7 | 8 |
...
| GPU rendering | wgpu (Metal/DX12/Vulkan) | Direct2D (Win), CoreGraphics, optional OpenGL | N/A | Skia or NanoVG (GL/GLES/Metal); HTML5 Canvas on Web | OpenGL (1/2/3/ES2), Cairo, NanoVG, Vulkan, Stub |
...
| | truce | JUCE | nih-plug | iPlug2 | DPF |
|---|---|---|---|---|---|
| macOS | All 7 formats | Yes | CLAP + VST3 | Yes | CLAP / VST3 / VST2 / AU / LV2 / standalone |
...
| iOS/Android/visionOS | No | Yes (iOS, Android) | No | Yes (iOS, visionOS) | No |
| Web | No | No | No | Yes | No |

## When to Use What

**truce** — You want Rust with maximum commercial-format coverage —
AU, AAX, and LV2 alongside CLAP/VST3/VST2 in a single codebase, a
combination not available in any other Rust framework. Hot reload,
batteries-included experience — built-in GUI, `cargo truce install`
/ `package` / `validate`, and signed cross-platform installers
(`.pkg` + Inno Setup `.exe`). Best for new Rust projects that need
the commercial-plugin trifecta of AU + AAX + LV2.
...
Audio (WAM) and visionOS app-extension targets, and a
self-contained graphics system. Good for cross-platform projects
targeting web/iOS/visionOS alongside desktop.
...
AAX. Well-established in the FOSS audio ecosystem (Ardour, LMMS,
Zynthian, Mod Devices). C++ with a lean API and multiple GUI
backends (OpenGL/GL3/GLES2, Cairo, NanoVG, Vulkan, Stub). DPF added
AU support recently; most DPF-based plugins are Linux-primary but
the framework runs on macOS and Windows too.
