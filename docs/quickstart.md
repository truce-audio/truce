...
This creates a standalone project with build tooling included:

```
my-gain/
├── Cargo.toml          # workspace
├── truce.toml          # signing + AU metadata
├── plugin/
│   ├── Cargo.toml      # your plugin crate
│   └── src/lib.rs      ← your plugin code lives here
└── xtask/              # build/install commands (cargo xtask)
```
...
generates the rest, including a `MyGainParamsParamId` enum with
typed IDs (e.g., `MyGainParamsParamId::Gain`) that can be used
instead of raw `u32` constants.
...
Update the layout to show it (using typed IDs generated by the
derive macro):
...
fn layout(&self) -> PluginLayout {
    PluginLayout::build("MY GAIN", "V0.1", vec![
        KnobRow { label: None, knobs: vec![
            KnobDef::knob(P::Gain, "Gain"),
            KnobDef::knob(P::Pan, "Pan"),
        ]},
    ], 80.0)
}
```
...
```bash
cargo xtask install --clap   # no --dev = static, zero overhead
```
...
```bash
cargo xtask install                # all formats
cargo xtask install --vst3         # VST3 (for Ableton, FL Studio)
cargo xtask install --au2          # AU v2 (for Logic Pro)
cargo xtask install --au3          # AU v3 (for Logic Pro, Ableton)
cargo xtask install --aax          # AAX (for Pro Tools, needs SDK)
...
fn layout(&self) -> PluginLayout {
    PluginLayout::build("MY GAIN", "V0.1", vec![
        KnobRow { label: None, knobs: vec![
            KnobDef::knob(P::Gain, "Gain"),
            KnobDef::slider(P::Pan, "Pan"),
        ]},
        KnobRow { label: Some("METERS"), knobs: vec![
            KnobDef::meter(&[Meter::Left.into()], "Level"),
        ]},
    ], 80.0)
}
```
...
**Run tests:**
```bash
cargo xtask test        # in-process plugin tests
