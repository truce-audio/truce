...
//! `mul_block`); that lives in `truce-example-gain-simd`'s envelope
...
impl PluginLogic for Gate {
    ...
    fn process(
        ...
        let threshold_lin = db_to_linear(self.params.threshold.read());
...
mod tests {
    ...
    fn gui_screenshot_macos() {
        truce_test::screenshot!(Plugin, "screenshots/gate_default_macos.png").run();
    }

    #[cfg(target_os = "linux")]
    #[test]
    fn gui_screenshot_linux() {
        truce_test::screenshot!(Plugin, "screenshots/gate_default_linux.png")
            .pixel_threshold(2)
            .run();
    }
    ...
    fn gui_screenshot_windows() {
        truce_test::screenshot!(Plugin, "screenshots/gate_default_windows.png")
            .pixel_threshold(2)
            .run();
    }
}
