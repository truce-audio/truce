//! MIDI arpeggiator - the "paranoid" (real-time-safe) version.
//!
//! Same arpeggiator as `truce-example-arpeggio-simple`, but it rebuilds
//! the arp sequence into pre-sized, reused buffers instead of a fresh
//! `Vec` each block, so `process` never allocates on the audio thread.
