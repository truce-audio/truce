...
/// Stateless descriptor - the tremolo's per-block DSP state is [`TremoloDspState`].
pub struct Tremolo;

#[derive(DspState)]
pub struct TremoloDspState {
...
impl PluginLogic for Tremolo {
    ...
    type DspState = TremoloDspState;

    fn init(_params: &TremoloParams) -> TremoloDspState {
        TremoloDspState {
            free_phase: 0.0,
            sample_rate: 44100.0,
        }
    ...
    fn reset(state: &mut TremoloDspState, params: &TremoloParams, config: &AudioConfig) {
