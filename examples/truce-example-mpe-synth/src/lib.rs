...
mod tests {
    ...
    /// End-to-end chain, no host: drive the spreader in Per-Note Vibrato
    /// mode and pipe its actual output straight into the synth. If the
    /// synth's voice swings, the MIDI 2.0 per-note pitch bend round-trips
    /// through truce end to end - so a real-world failure (e.g. in Logic)
    /// is the host dropping per-note messages, not the framework.
    #[test]
    fn spreader_per_note_vibrato_detunes_the_synth_voice() {
        use truce_example_mpe_spreader::{Algo, Spreader, SpreaderParams};

        let spreader_params = Arc::new(SpreaderParams::new());
        spreader_params.algo.set_value(Algo::PerNoteVibrato);
        let mut spreader = Spreader::new(Arc::clone(&spreader_params));
        spreader.reset(44100.0, 64);

        let synth_params = Arc::new(SynthParams::new());
        let mut synth = Synth::new(Arc::clone(&synth_params));
        synth.reset(44100.0, 64);

        let transport = TransportInfo::default();
        let mut peak_bend = 0.0_f64;

        // Hold one note across enough blocks for the vibrato LFO to swing.
        for blk in 0..64u32 {
            // Spreader: MIDI effect, no audio buffers. Note-on once.
            let spread_in_refs: Vec<&[f32]> = Vec::new();
            let mut spread_out_refs: Vec<&mut [f32]> = Vec::new();
            let mut spread_buf =
            ...
            spreader.process(&mut spread_buf, &spread_in, &mut spread_ctx);

            // Synth: consume the spreader's promoted output verbatim.
            let sink_in_refs: Vec<&[f32]> = Vec::new();
            let mut l = vec![0.0f32; 64];
            let mut r = vec![0.0f32; 64];
            let mut sink_out_refs: Vec<&mut [f32]> = vec![&mut l[..], &mut r[..]];
            let mut sink_buf =
                unsafe { AudioBuffer::from_slices(&sink_in_refs, &mut sink_out_refs, 64) };
            let mut sink_out_ev = EventList::default();
            let mut sink_ctx = ProcessContext::new(&transport, 44100.0, 64, &mut sink_out_ev);
            synth.process(&mut sink_buf, &spread_out, &mut sink_ctx);

            if let Some(v) = synth
                .voices
                .iter()
                .find(|v| v.active && v.channel == 0 && v.note == 60)
            {
                peak_bend = peak_bend.max(v.bend_semis.abs());
            }
        }

        assert!(
            peak_bend > 0.01,
            "spreader per-note vibrato should detune the synth voice; peak |bend| \
             was {peak_bend} semitones (0 = the per-note bend never reached the synth)"
        );
    }
