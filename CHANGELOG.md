...
- iOS editor thunks (tick, touch, and keyboard selectors on the built-in, egui, iced, and slint backends) catch panics at the UIKit boundary, so a panic in author view code can't abort the AUv3 host. A caught editor panic no longer poisons the editor's mutex (recovered with `into_inner`) - on the iOS egui / iced / slint editors and the desktop egui / built-in editors alike - so the editor keeps rendering instead of freezing after one caught panic, and the built-in editor's close-path teardown still runs (no leaked wgpu surface).
- Editor `request_resize` reaching a GUI callback re-entrantly - directly on AU, or via a host that answers VST3 `resizeView`/`onSize` synchronously - no longer re-enters the wrapper's editor cell. Both defer via `PluginCell::try_enter` and apply on the next size query; the debug overlap detector reports same-thread re-entry distinctly from a genuine host overlap.
...
- `bus_layouts()` now defaults to `BusLayout::stereo_and_mono()` (stereo and mono) instead of stereo-only. The default ensures audio effects show up on both **mono and stereo tracks** in Logic Pro (and other hosts that filter the insert menu by channel format) without the plugin declaring anything. You can still override it - return `vec![BusLayout::stereo()]` for a stereo-only effect, or any set you want. Behavior change: with the new default a mono track hands `process` a 1-channel buffer, so loop over `buffer.channels()` rather than assuming two (the shipped examples do; instruments and MIDI effects override the default as before).
- Offer the host more than one channel layout: return several from `bus_layouts()` and the host picks the one its track needs. This works on every format that can renegotiate ports - CLAP (`audio-ports-config`), VST3 (`setBusArrangements`, now tracked per instance so two instances on different-width tracks don't report each other's arrangement), AU (the channel-capability property, plus one AU v3 bus per layout), and AAX (one component per layout). VST2 and LV2 stay on the first layout.
...
- The standalone host runs the exact `(in, out)` of the selected layout (`--bus-layout <index>`, `--list-bus-layouts` to enumerate, or the Settings > Bus Layout menu). It keeps the audio device at a width the hardware supports and maps the plugin onto it, so a mono layout plays through a stereo-only output instead of failing with "device doesn't offer 1 output channels", and an asymmetric layout (mono-in / stereo-out) runs at its real dimensions rather than a collapsed width.
- Helpers for authoring across widths: `BusLayout::mono()` / `BusLayout::stereo_and_mono()` constructors, and `AudioBuffer::for_each_frame_io::<IN, OUT>` (with a `for_each_stereo_frame` shorthand for the `(2, 2)` case) to run a fixed-shape DSP - a `reverb_stereo` graph, a stereo filter block - over any declared bus: a mono input fans into both graph inputs, a stereo bus maps 1:1, no per-width branch.
...
- Aspect-locked editors in the Linux standalone now hold their ratio during an edge-drag instead of letterboxing: the outer window's X11 size hints gained the missing `PAspect` constraint, so the window manager keeps the drag on ratio the way Windows (`WM_SIZING`) and macOS (`setContentAspectRatio:`) already did. Affects backends whose `set_size` takes any size verbatim (egui / iced / Slint); honored by floating WMs, ignored by tiling ones as with the existing min/max hints.
...
- **New `Editor` trait methods.** `size_increment() -> Option<(u32, u32)>` (interactive-resize granularity in logical points; the X11 standalone maps it onto `PResizeInc` so window-edge drags snap), `aspect_ratio() -> Option<(u32, u32)>` (`(num, denom)`; CLAP / VST3 / AU v3 / standalone honor it, VST2 / LV2 / AAX silently ignore), and `prefers_pow2() -> bool` (renderer wants power-of-two surface sizes). All have defaults so existing plugins compile unchanged. Matching builder methods on every editor type; iOS variants are no-op stubs.
...
- **Windows: wgpu editors pin the DX12 shader compiler to FXC.** Fixes a blank editor (egui, built-in `GpuEditor`) and a host crash (Slint) in Pro Tools. wgpu 29 defaults DX12 to a dynamically-loaded **DXC** (`dxcompiler.dll`); when the host process has already loaded its own incompatible `dxcompiler.dll` (Pro Tools does), `DxcCreateInstance` returns `E_NOINTERFACE` and the *entire* DX12 backend fails to initialize - the wgpu instance ends up with zero adapters, so surface creation fails (white editor, or a panic on Slint's `.expect`). The `truce-gpu`, `truce-gui`, `truce-egui`, and `truce-slint` editor instances now request `Dx12Compiler::Fxc` (`d3dcompiler_47.dll`, always present on Windows, never conflicts). iced (wgpu 0.19, FXC by default) and vizia (OpenGL) were never affected. The Windows editor instances also narrow to `Backends::DX12` (the only backend feature compiled in on Windows). Offscreen/headless screenshot paths are unchanged.
