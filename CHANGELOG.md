...
- **The standalone routes a MIDI device per port.** `--midi-input` is repeatable; the i-th device maps to the plugin's MIDI input port i (`--midi-input Keystep --midi-input Launchkey`). The macOS and Windows menu bars grow a "MIDI Input - Port k" device picker per port so devices can be assigned live (up to four ports in the menu; the CLI has no cap). Devices past the plugin's port count warn and are ignored; the computer keyboard stays on port 0. Previously the standalone delivered everything on port 0.
