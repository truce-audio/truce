...
- **`PluginExport` grew `meter_store()`.** Only affects hand-written `PluginExport` impls (the `truce::plugin!` shells provide it); return a `MeterStore` handle kept alongside the params `Arc`. **Migration:** `fn meter_store(&self) -> Arc<MeterStore> { Arc::clone(&self.meters) }`.
