...
- **Instance mediation**: every wrapper owns its plugin behind a `Mutex` (the audio thread locks per block); editors read meters from a shared atomic `MeterStore` and state via a bounded blocking read. Keep `save_state` cheap - a mid-block save delays the next block. (#175)
