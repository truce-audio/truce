...
- Audio effects now appear on **mono tracks** too. The default `bus_layouts()` is `BusLayout::stereo_and_mono()` (stereo and mono) instead of stereo-only, so an effect that declares no layouts of its own is offered on both widths. Behavior change: a mono track now hands `process` a 1-channel buffer, so loop over `buffer.channels()` rather than assuming two (the shipped examples do; instruments and MIDI effects override the default as before).
