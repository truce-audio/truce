...
- **Multiple MIDI ports.** `Event` gains a `port` field and `truce.toml` gains `midi_input_ports` / `midi_output_ports`. A plugin filters inbound events by `event.port` and stamps outbound ones with the target port. CLAP exposes N note ports and routes by port index; VST3 / LV2 / VST2 / AU / AAX carry one and log a skip when more are declared. Single-port plugins are unchanged (`port` is always `0`; construct events with `Event::new`).
