...
- **A stuck graphics driver no longer freezes the DAW when an egui editor opens (Windows).** wgpu device setup used to run inline on the host's GUI thread, where a wedged driver can block it forever. It now runs on a worker thread with a bounded wait (`TRUCE_GPU_INIT_TIMEOUT_MS`, default 3000 ms); on timeout the editor opens blank, stays responsive, and adopts the renderer if it ever arrives.
- **Plugin editors no longer bog down the host's GUI thread on Windows.** Painting faster than the compositor displays an embedded editor blocks in the swapchain acquire (~74 ms per paint in a REAPER FX window), so editors with meters kept the host's GUI thread near-saturated - felt as the DAW hanging, worst while resizing. The egui, iced, Slint, and built-in editors now pace paints to the measured compositor rate; egui also debounces swapchain reconfigures and pauses painting briefly around size changes. (vizia's frame loop lives in upstream `vizia_baseview` and is not covered.)
- **A vizia editor no longer crashes the DAW when hardware OpenGL is unavailable (Windows).** Without a working GPU driver (or in a remote session), baseview's GL bootstrap panicked inside the Win32 window proc and aborted the host. The editor now probes for the required WGL extensions first and stays closed, with an error log, when they're missing.
- **Editor size constraints are honored even when the host bypasses resize negotiation.** Some hosts resize the plugin's embedded window directly at the windowing-system level (Bitwig on X11), skipping VST3 `checkSizeConstraint` / CLAP resize hints - so `min_size` / `max_size` / `aspect_ratio` were silently ignored. Every GUI backend's resize handler now fits the incoming size to the editor's declared bounds and pushes a corrective resize back (at most once per offending size, so an uncooperative host can't be spun into a feedback loop); a non-resizable editor reports its natural size as both min and max so it stays pinned rather than ratcheting larger. When a host still forces the window past the editor's max (or off its locked aspect), the editor renders at its fitted size anchored top-left with the surrounding area filled black instead of stretching to a blurry fill - reflowing to fill within `[min, max]` as before (the built-in editor centers its letterbox). Applies to egui / iced / Slint / built-in. (#163)
