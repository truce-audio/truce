...
- **Per-note conventions are shared across formats** so the same event sounds the same everywhere: `PerNotePitchBend` full-scale is ±48 semitones (MPE convention; wider host bends saturate), and per-note volume crosses in the `0..=4` linear-gain domain CLAP and VST3 both define (unity gain at the wire's quarter point).
...
- `EventBody` and `TransportInfo` derive `PartialEq`; `truce_core::midi` gained `upconvert_to_midi2` (spec min-center-max upscaling), the inverse of `downconvert_to_midi1`.
