...
- **AU v3** declares `audioUnitMIDIProtocol` from the input dialect and drains output through the host's UMP `MIDIEventList` block whenever the host supplies it, for every output dialect (macOS 12+ / iOS 15+; the byte block is the fallback). The list is declared in the host's `hostMIDIProtocol` (default 2.0) and carries a protocol-pure stream - all MT 0x4 channel voice in 2.0 lists, all MT 0x2 in 1.0 lists, events converting across dialects as needed, per the UMP spec's no-mixing rule. SysEx output rides the same stream as SysEx-7 packet chains (legal in both protocols).
- **VST3 has no UMP, so it gets two translations.** Per-note control rides note expression: `PerNoteCC` (volume/pan/vibrato/expression/brightness) and `PerNotePitchBend` round-trip to/from `kNoteExpressionValueEvent` - lossy, but the MPE subset survives. Output is keyed by a deterministic `noteId`; input resolves the host's note-on `noteId`s back to `(channel, note)` and carries values at full `f64` precision; the component declares `INoteExpressionController` so hosts offer per-note input at all. Per-note CCs with no predefined expression type degrade to channel CCs, as on CLAP.
- **Channel-level VST3 MIDI input rides hidden proxy parameters.** VST3 delivers CC / pitch bend / channel pressure only to `IMidiMapping` parameters, so a plugin consuming MIDI *events* heard nothing on VST3 while the same build heard everything elsewhere - every MIDI-accepting plugin now advertises 16 channels x 130 hidden, non-automatable mapping targets *per MIDI input port* that decode straight back into events on that port, so multi-timbral plugins keep each bus's controllers separate. Explicit `midi_cc`-style bindings keep priority for their controller on every bus; program change stays binding-only; proxies never enter saved state.
- **Per-note conventions are shared across formats** so the same event sounds the same everywhere: `PerNotePitchBend` full-scale is ±48 semitones (MPE convention; wider host bends saturate), and per-note volume crosses in the `0..=4` linear-gain domain CLAP and VST3 both define (unity gain at the wire's quarter point). Only *registered* per-note controllers map onto note expressions and 1.0 channel CCs - assignable indices are manufacturer-defined and never alias onto them (they ride natively where UMP exists, and drop elsewhere).
...
- **A port count is authoritative.** `midi_input_ports = 2` alone enables MIDI input identically on every format (capability is `count > 0`, resolved once and baked onto `PluginInfo`); a count that contradicts `midi_input` / `midi_output` is a compile error, and `cargo truce` rejects it at config load.
...
- **The standalone routes a MIDI device per port.** `--midi-input` is repeatable; the i-th device maps to the plugin's MIDI input port i (`--midi-input Keystep --midi-input Launchkey`). The macOS and Windows menu bars grow a "MIDI Input - Port k" device picker per port so devices can be assigned live (up to sixteen ports in the menu, identically on both platforms; the CLI has no cap). Devices past the plugin's port count warn and are ignored; the computer keyboard stays on port 0. Previously the standalone delivered everything on port 0.
...
- **GUI/host access to the plugin instance is mediated.** Editor `get_meter` closures read a shared atomic-slot `MeterStore` (the LV2 pattern, now on every format), and every wrapper owns its plugin behind a `Mutex`: the audio thread locks per block (uncontended: one CAS), host state saves lock for the serialization, editor state reads block for the (bounded) remainder of the in-flight audio block. Previously those paths dereferenced a raw pointer to the instance while `process()` held `&mut` - formally UB, and a trap for `get_meter` / `save_state` overrides reading plain fields. The store rides a new `PluginExport::meter_store()` (provided by the `truce::plugin!` shells; only a hand-written `PluginExport` impl would notice). The visible contract change: a session save landing mid-block delays the *next* block by the remainder of `save_state`, so keep `save_state` cheap (copy out, don't compute). (#175)
- **Miri + fuzzing coverage for the pure-Rust core.** `cargo miri test` runs green on truce-utils / truce-params / truce-core (Stacked Borrows, the buffer transmutes included), a two-thread mediation repro runs under `-Zmiri-many-seeds`, and a `fuzz/` crate carries libFuzzer targets for the state envelope, `.trucepreset` container, MIDI/UMP decoders, and SysEx reassembly - each with round-trip/agreement oracles, plus a corpus replay binary that re-runs fuzz findings under Miri. New `Miri` (PR gate) and `Fuzz` (weekly) workflows.
...
- **The hot-reload ABI canary is epoch-versioned.** Sizes can't see every layout change - `Event::port` landed in former padding, so a stale 1.x shell would read uninitialized padding as the port with every size field matching. `AbiCanary` gained a hand-bumped `ABI_EPOCH` (now 2) for exactly that class, and the canary export symbol is versioned (`truce_abi_canary_v2`) because the struct crosses the shell/dylib boundary by value - a canary-layout mismatch now fails at symbol lookup instead of corrupting the return slot. Stale pre-2.0 logic dylibs are refused cleanly.
- `EventBody` and `TransportInfo` derive `PartialEq`; `truce_core::midi` gained `upconvert_to_midi2` and the public spec up-scalers behind it (`upscale_7_to_16` / `upscale_7_to_32` / `upscale_14_to_32` - exact center and max, unlike a linear rescale), the inverse of `downconvert_to_midi1`.

### Fixes

- **AU v2 answers `kAudioUnitProperty_ParameterStringFromValue`.** Generic host views and control-surface displays (Logic's controls view, MCU scribble strips) now show truce-formatted values ("50%", "-12.0 dB", enum variant names) instead of raw plain numbers; parameters advertise `ValuesHaveStrings`. AU v3 already routed its tree's string callback the same way.
- **LV2 no longer declares `units:pc` on percent parameters.** truce stores percent plain values as `0..=1` fractions, but `units:pc` tells hosts the raw value already is percent - Ardour rendered full mix as "1%". The value now shows unitless rather than mislabeled.
- **AU shim callback ABI is append-only again, with a version handshake.** The `output_ump_*` callbacks had been inserted mid-`AuCallbacks`, shifting every later field (including the append-only factory-preset and SysEx-input callbacks) - a newer AU v3 appex generated by `cargo truce` could call the wrong offset on an older plugin binary. They moved to the struct tail, and `AuCallbacks` gained a fixed-offset `abi_version` field (mirrored `#define` + Rust const, lockstep-tested) so a newer appex can tell how far the callback tail extends before invoking it.
- **`cargo truce run` builds the standalone `--no-default-features`.** The preview binary needs no format wrapper; linking clap / vst3 / au / aax / lv2 only bloated it and ran their load-time registration constructors (e.g. the AU shim's MIDI-port-clamp warning) in a host that never uses them. Playback stays on (the build enables `truce-standalone/playback` directly, so `--input-file` / `--output-file` / `--no-playback` still work regardless of how a plugin routes that feature), and a new `--features` flag re-adds any others you want alongside `standalone`.
...
- **CLAP: wildcard (`-1` key/channel) `NOTE_OFF` / `NOTE_CHOKE` expand instead of dropping.** note_id-addressing hosts release notes with wildcard addresses, which previously never reached the plugin - the voice rang forever. The wrapper tracks sounding notes per port (a 16x128 bitset, maintained from the note-event stream) and synthesizes a `NoteOff` for each sounding note the wildcard matches, so the expansion is bounded by real polyphony.
...
- **Editor size constraints are honored even when the host bypasses resize negotiation.** Some hosts resize the plugin's embedded window directly at the windowing-system level (Bitwig on X11), skipping VST3 `checkSizeConstraint` / CLAP resize hints - so `min_size` / `max_size` / `aspect_ratio` were silently ignored. Every GUI backend's resize handler now fits the incoming size to the editor's declared bounds and pushes a corrective resize back (at most once per offending size, so an uncooperative host can't be spun into a feedback loop); a non-resizable editor reports its natural size as both min and max so it stays pinned rather than ratcheting larger. When a host still forces the window past the editor's max (or off its locked aspect), the editor renders at its fitted size anchored top-left with the surrounding area filled black instead of stretching to a blurry fill - reflowing to fill within `[min, max]` as before (the built-in editor centers its letterbox). Applies to egui / iced / Slint / built-in. The Windows and macOS standalones now also enforce min / max (and the aspect lock, for interactive drags) on their own window frames - Windows via a `WM_GETMINMAXINFO` / `WM_SIZING` subclass that also covers programmatic resizes, macOS via `contentMinSize` / `contentMaxSize` / `contentAspectRatio` (which also bound the zoom button) plus a per-frame snap-back for the programmatic `setFrame:` calls that bypass them - previously only backends whose `set_size` clamps kept the frame honest. (#163)
